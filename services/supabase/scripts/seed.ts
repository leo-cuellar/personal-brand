        ...
        for (const publicationIdea of mockPublicationIdeas) {
            await client`
                INSERT INTO public.publication_ideas (
                    id, personal_brand_id, title, description, link, status, source, source_summary, metadata, created_at, updated_at, is_archived
                ) VALUES (
                    ${publicationIdea.id}::uuid,
                    ${publicationIdea.personalBrandId}::uuid,
                    ${publicationIdea.title},
        ...
        for (const person of mockPersons) {
            ...
            await client`
                INSERT INTO public.personal_brands (
                    id, name, username, niche, scheduler_id, social_accounts, brand_narrative, strong_opinions, values,
        ...
        for (const persona of mockBuyerPersonas) {
            ...
            await client`
                INSERT INTO public.buyer_personas (
                    id, personal_brand_id, name, description, goals, frustrations, desires,
                    created_at, updated_at, is_archived
                ) VALUES (
                    ${persona.id}::uuid,
